...
from deeptutor.agents.chat.agent_loop import InlineThinkFilter, _is_temperature_unsupported
...
class TestTemperatureUnsupportedDetection:
    """The reactive fallback that catches reasoning models not in the static
    override list, when a provider rejects a non-default temperature."""

    def test_detects_openai_reasoning_model_error(self) -> None:
        # The exact 400 body reported from the demo backend.
        exc = Exception(
    ...
    def test_ignores_unrelated_errors(self) -> None:
        assert _is_temperature_unsupported(Exception("rate limit exceeded")) is False
        # An error mentioning temperature but not an unsupported-value rejection.
        assert _is_temperature_unsupported(Exception("temperature must be a float")) is False

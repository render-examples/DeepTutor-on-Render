...
async def test_mineru_settings_roundtrip_redacts_token(
    ...
    service = RuntimeSettingsService(
        tmp_path / "settings", process_env={"MINERU_API_TOKEN": "secret-token"}
    )
    monkeypatch.setattr(settings_router, "get_runtime_settings_service", lambda: service)
    ...
    # The token is environment-only: only a boolean + the env var name leave
    # the backend, and the raw value is never echoed.
    assert response["api_token_set"] is True
    assert response["api_token_env_var"] == "MINERU_API_TOKEN"
    ...
    # Resolves from the env but is never persisted to disk.
...
async def test_update_catalog_invalidates_runtime_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Response is the key-free catalog with env indicators, not the raw body.
    assert "new-llm-key" not in str(response)
    assert "new-embedding-key" not in str(response)
    resp_llm = response["catalog"]["services"]["llm"]["profiles"][0]
    assert "api_key" not in resp_llm
    assert "api_key_set" in resp_llm
...
async def test_apply_catalog_invalidates_runtime_caches(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # Response is the key-free catalog with env indicators, not the raw body.
    assert "after-apply-llm-key" not in str(response)
    assert "after-apply-embedding-key" not in str(response)
    assert "api_key" not in response["catalog"]["services"]["llm"]["profiles"][0]
...
async def test_fetch_models_returns_picker_options(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # The key is resolved from OPENAI_API_KEY, not the request body.

...
def test_mineru_defaults_and_normalization(tmp_path: Path) -> None:
    ...
    # api_token is environment-only (MINERU_API_TOKEN); never persisted.
...
def test_mineru_process_env_override(tmp_path: Path) -> None:
    ...
    # api_token resolves from MINERU_API_TOKEN, never from disk.
    assert effective["api_token"] == "env-token"
    assert effective["local_cli_path"] == "/env/bin/mineru"
    # The persisted file keeps the on-disk non-secret values (nested under
    # engines.mineru), not the env overrides; the token is never written.
...
def test_document_parsing_v1_to_v2_migration(tmp_path: Path) -> None:
    ...
    # Hard-cut migration: a legacy on-disk token is stripped (env-only now).
    assert mineru["api_token"] == ""

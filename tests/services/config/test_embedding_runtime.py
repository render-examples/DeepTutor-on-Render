...
def test_embedding_local_vllm_uses_env_key(monkeypatch) -> None:
    monkeypatch.setenv("VLLM_API_KEY", "local-secret")
    catalog = _build_catalog(
        embedding_profile={
            "id": "embedding-p",
            "name": "Embedding",
...
def test_embedding_openrouter_env_key(monkeypatch) -> None:
...
def test_embedding_provider_env_key(monkeypatch) -> None:

...
def test_llm_gateway_via_explicit_binding(monkeypatch) -> None:
    # Secrets are environment-only, so gateway detection by raw key prefix is
    # no longer possible (we don't store or sniff the key). The supported path
    # is an explicit gateway binding plus its env var.

...
def test_config_from_entry_requires_server_url(monkeypatch) -> None:
    # The API key is environment-only (LIGHTRAG_API_KEY); a key on the entry is
    # ignored, and the value comes from the environment instead.
    monkeypatch.setenv("LIGHTRAG_API_KEY", "k")
    cfg = config_from_entry({"server_url": "http://x:9621/", "api_key": "disk-ignored"})

...
class ProviderCliDocsContractTest(unittest.TestCase):
    ...
    def test_readmes_match_the_cli_contract(self) -> None:
        ...
        # The root README on this Render fork is a deploy guide and intentionally
        # does not document the CLI's provider-auth commands, so we don't assert
        # positive CLI-usage prose there. We do still guard against a doc ever
        # mislabeling github-copilot as an OAuth *login* (it only validates an
        # existing session); the authoritative behavior is pinned against the CLI
        # source in test_provider_contract_describes_copilot_as_validation_not_oauth_login.

...
@pytest.mark.parametrize(
    ...
    [
        "gpt-5",
        "gpt-5-mini",  # GA mini reasoning model
        "gpt-5.1",
        "gpt-5.6-luna",  # the out-of-the-box default (see config/defaults.py); covered by the "gpt-5" prefix
        "o1",
        "o1-preview",
        "o3",
        "o3-mini",
        "o4-mini",  # previously only caught by the reactive 400 retry
    ],
)
def test_agentic_kwargs_force_default_temperature_for_reasoning_models(model) -> None:
    # Reasoning models reject any non-default temperature with a 400. The kwargs
    # builder must force the model's fixed value (1.0) rather than pass 0.2, and
    # must send the token cap as max_completion_tokens (these models reject
    # max_tokens too).
    kwargs = build_completion_kwargs(
        temperature=0.2,
        model=model,
...
def test_agentic_kwargs_keep_requested_temperature_for_regular_models() -> None:
    kwargs = build_completion_kwargs(
        temperature=0.2,
        model="gpt-4o",
        max_tokens=1024,

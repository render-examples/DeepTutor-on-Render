...
async def get_system_status():
    ...
    try:
        ...
        # "configured" must reflect a usable credential, not merely a resolvable
        # model. A cloud provider with no API key resolved would otherwise report
        # ready and then 401 on the first turn. OAuth providers legitimately
        # carry no key, so they count as configured.
        has_credential = bool(llm_config.api_key) or llm_config.provider_mode == "oauth"
        if has_credential:
            result["llm"]["status"] = "configured"
        else:
            result["llm"]["status"] = "not_configured"
            result["llm"]["testable"] = False
            result["llm"]["error"] = (
                "No API key resolved for the active LLM provider. "
                "Set the provider's key in the environment (e.g. OPENAI_API_KEY)."
            )

...
class PageIndexConfigUpdate(BaseModel):
    # The API key is environment-only (PAGEINDEX_API_KEY) and is never accepted
    # or persisted here; only the non-secret base URL is editable.
    api_base_url: str | None = None


def _pageindex_config_payload() -> dict:
    """PageIndex pipeline settings for the UI; the key is env-only (a boolean)."""
    ...
    return {
        ...
        "api_key_env_var": "PAGEINDEX_API_KEY",
...
async def update_pageindex_pipeline_config(payload: PageIndexConfigUpdate):
    ...
    try:
        ...
        # The key is environment-only; save_pageindex never persists it.
        service.save_pageindex({"api_base_url": api_base_url})
        return _pageindex_config_payload()
    except Exception as e:
...
class ProbeLightRagServerRequest(BaseModel):
    # The API key is environment-only (LIGHTRAG_API_KEY); never accepted here.
    server_url: str
...
async def probe_lightrag_server_route(payload: ProbeLightRagServerRequest):
    ...
    from deeptutor.services.rag.pipelines.lightrag_server.config import LIGHTRAG_API_KEY_ENV
    ...
    result = await probe_server(server_url, resolve_secret(LIGHTRAG_API_KEY_ENV))
    return result.to_dict()

...
from deeptutor.services.config.env_secrets import (
    env_vars_for,
    resolve_secret,
    secret_is_set,
)
...
class MinerUSettingsUpdate(BaseModel):
    ...
    The MinerU API token is environment-only (``MINERU_API_TOKEN``) and is never
    accepted or persisted from the UI, so no ``api_token`` field is exposed
    here. The GET payload reports only ``api_token_set``, never the raw token.
    """
...
class DocumentParsingUpdate(BaseModel):
    ...
    carries partial per-engine updates merged over the stored slices. Secrets
    (e.g. the MinerU ``api_token``) are environment-only and never persisted
    from here — any inbound token is dropped on normalize. The MinerU engine's
    own knobs can also be edited via the legacy ``/mineru`` endpoints; both
    preserve the other engines' settings.
    """
...
def _catalog_payload(catalog: dict[str, Any] | None = None) -> dict[str, Any]:
    """The key-free catalog augmented with per-profile env-var indicators.

    Secrets are environment-only, so the catalog on disk never carries a key.
    For the UI we add ``api_key_set`` (does the provider's env var hold a
    value) and ``api_key_env_var`` (the var name to display), mirroring the
    MinerU/PageIndex ``*_set`` pattern. Never echoes a raw key.
    """
    catalog = catalog if catalog is not None else get_model_catalog_service().load()
    for service_name, service in (catalog.get("services") or {}).items():
        for profile in service.get("profiles", []) or []:
            # Defensive: strip anything a client may have smuggled in.
            profile.pop("api_key", None)
            env_vars = env_vars_for(service_name, profile)
            profile["api_key_env_var"] = env_vars[0] if env_vars else ""
            profile["api_key_set"] = secret_is_set(*env_vars)
    return catalog
...
async def get_catalog():
    ...
    return {"catalog": _catalog_payload()}
...
def _mineru_settings_payload() -> dict[str, Any]:
    ...
    return {
        ...
        "api_token_env_var": "MINERU_API_TOKEN",
...
async def update_mineru_settings(payload: MinerUSettingsUpdate):
    _require_settings_admin()
    service = get_runtime_settings_service()
    # The API token is environment-only (MINERU_API_TOKEN); save_mineru never
    # persists it, so we do not accept one from the request body.
    service.save_mineru(
        {
            "mode": payload.mode,
...
async def test_mineru_connection(payload: MinerUSettingsUpdate):
    ...
    service = get_runtime_settings_service()
    # Secrets are environment-only: resolve the token from MINERU_API_TOKEN
    # (via process overrides) and ignore any token in the request body.
    token = service.load_mineru(include_process_overrides=True).get("api_token", "")
    config = MinerUConfig(
        mode="cloud",
...
async def update_catalog(payload: CatalogPayload):
    _require_settings_admin()
    get_model_catalog_service().save(payload.catalog)
...
async def fetch_models_from_provider(payload: FetchModelsPayload):
    ...
    populate a model picker from ``base_url`` (the key is resolved from the
    binding's environment variable) instead of making the user type model IDs
    by hand.
    """
    _require_settings_admin()
    ...
    # Secrets are environment-only: resolve from the binding's env var and
    # ignore any key in the request body.
    api_key = resolve_secret(*env_vars_for("llm", {"binding": binding}))
    try:

...
def _build_cors_settings() -> dict[str, object]:
    ...
    # Safe by default: a fresh (public) deploy only trusts localhost + any
    # explicit CORS_ORIGIN(S). Because the middleware sends
    # `allow_credentials=True`, a wildcard origin would let any website make
    # credentialed cross-origin requests, so it must be opted into explicitly.
    # Set CORS_ALLOW_ALL=true to restore the old permissive LAN behavior (mirror
    # of pre-v1.3.8) on a trusted network.
    allow_all = bool(system_settings.get("cors_allow_all"))
    allow_origin_regex = r"https?://.*" if allow_all else None
    mode = "permissive" if allow_all else "explicit"
    return {
...
async def demo_rate_limit(request, call_next):
    from deeptutor.services.demo import client_ip, get_demo_limiter

    ...
    async def _create_response_stream(
        ...
        try:
            ...
            if "temperature" in kwargs and _is_temperature_unsupported(exc):
                # Reasoning models only accept the default temperature. Known
                # families are forced up-front (see build_completion_kwargs), but
                # this catches models not in the static override list: drop the
                # field so the provider applies its default.
                retry_kwargs = dict(kwargs)

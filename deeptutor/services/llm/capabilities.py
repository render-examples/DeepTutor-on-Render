...
MODEL_OVERRIDES: dict[str, dict[str, object]] = {
    ...
    # Reasoning models — only support temperature=1.0 (any other value 400s).
    # Prefix-matched (case-insensitive startswith), so "gpt-5" also covers
    # gpt-5-mini / gpt-5-nano and the gpt-5.1 / gpt-5.6-{sol,terra,luna} lines;
    # "o1" / "o3" / "o4" cover their -mini / -preview variants. The default LLM
    # (gpt-5-mini, see config/defaults.py) relies on the gpt-5 entry here.
    # See: https://github.com/HKUDS/DeepTutor/issues/141
    "gpt-5": {
        "forced_temperature": 1.0,
        # GPT-5 accepts image input; assert it here so the family is
        # vision-capable even under OpenAI-compatible proxies (binding
        # "custom"), whose provider-level default is supports_vision=False.
        "supports_vision": True,
    },
    "o1": {
        "forced_temperature": 1.0,

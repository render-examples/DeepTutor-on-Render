...
from .env_secrets import generic_env_var_for, llm_env_vars, resolve_secret
...
def _collect_provider_pool(catalog: dict[str, Any]) -> dict[str, NormalizedProviderConfig]:
    ...
    for profile in llm_profiles:
        ...
        providers[name] = NormalizedProviderConfig(
            name=name,
            api_key=resolve_secret(*llm_env_vars(name)),
...
def resolve_llm_runtime_config(
    ...
    *,
    ...
    active_api_key = resolve_secret(*llm_env_vars(binding_hint))
    ...
    # The binding hint may be empty (no configured profile) while the resolved
    # spec is a concrete cloud provider (e.g. openai via find_by_model). In that
    # case active_api_key was never looked up for the *resolved* provider — do
    # it now so a real OPENAI_API_KEY in the environment is honored instead of
    # falling through to the local-only sentinel and 401'ing.
    if not api_key and not (spec.is_local or spec.is_oauth):
        api_key = resolve_secret(*llm_env_vars(spec.name))
...
def _collect_embedding_provider_pool(
    ...
    for profile in embedding_profiles:
        ...
        providers[name] = NormalizedProviderConfig(
            name=name,
            api_key=resolve_secret(generic_env_var_for(name)),
            api_base=_as_str(profile.get("base_url")) or None,
            api_version=_as_str(profile.get("api_version")) or None,
            extra_headers=_to_headers(profile.get("extra_headers")) or None,
        )
...
def resolve_embedding_runtime_config(
    ...
    *,
    ...
    active_api_key = resolve_secret(generic_env_var_for(binding_hint))
    active_api_base = _as_str((profile or {}).get("base_url"))
    active_api_version = _as_str((profile or {}).get("api_version"))
    active_extra_headers = _to_headers((profile or {}).get("extra_headers"))
    ...
    if not api_base and spec.default_api_base:
        api_base = spec.default_api_base
    api_version = active_api_version or ((mapped.api_version or "") if mapped else "")
    # As with the LLM resolver: the binding hint may be empty while the provider
    # was resolved by model keyword. Read the resolved provider's env key so a
    # real key in the environment is honored rather than left blank.
    if not api_key and not spec.is_local:
        api_key = resolve_secret(generic_env_var_for(provider_name))
    extra_headers = active_extra_headers or ((mapped.extra_headers or {}) if mapped else {})
...
def resolve_tts_runtime_config(
    ...
    *,
    ...
    api_key = resolve_secret(generic_env_var_for(provider))
    if not api_key and spec.is_local:
        api_key = "sk-no-key-required"
...
def resolve_search_runtime_config(
    ...
    *,
    ...
    api_key = resolve_secret(generic_env_var_for(requested_provider))

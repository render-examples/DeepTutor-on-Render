...
def _default_llm_service() -> dict[str, Any]:
    # Seed an active OpenAI profile so a fresh deploy boots already configured:
    # once OPENAI_API_KEY is set the app can chat immediately, with no manual
    # Settings > Models step. No secret is stored here — the key is resolved
    # from the environment at runtime (see env_secrets / _normalize strips keys).
    return {
        "active_profile_id": "llm-openai-default",
        "active_model_id": "llm-openai-gpt-5-6-luna",
        "profiles": [
            {
...
def _default_embedding_service() -> dict[str, Any]:
    # Seed a matching OpenAI embedding profile so knowledge bases / RAG index
    # out of the box (OPENAI_API_KEY covers embeddings too).
    return {
        "active_profile_id": "embedding-openai-default",
        "active_model_id": "embedding-openai-3-small",
        "profiles": [
            {
...
def _default_catalog() -> dict[str, Any]:
    return {
        "version": 1,
        "services": {
            "llm": _default_llm_service(),
            "embedding": _default_embedding_service(),
    ...
    def _normalize(self, catalog: dict[str, Any]) -> bool:
        ...
        for service_name in ("llm", "embedding", "search", "tts", "stt", "imagegen", "videogen"):
            ...
            for profile in profiles:
                # Secrets are environment-only: never persist or accept a key
                # from disk. Stripping here doubles as the hard-cut migration
                # for any legacy catalog that still carries a raw key.
                if profile.pop("api_key", None):
        ...
        if stripped_secret:
            logger.warning(
                "Stripped a raw API key from the model catalog. Secrets are "
                "sourced only from environment variables and are never stored "
                "on disk; set the provider's env var instead."
            )
        return changed

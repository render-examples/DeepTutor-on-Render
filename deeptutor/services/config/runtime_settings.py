...
DEFAULT_SYSTEM_SETTINGS: dict[str, Any] = {
    ...
    "cors_origin": "",
    "cors_origins": [],
    # Allow credentialed cross-origin requests from ANY origin. Off by default:
    # a fresh (public) deploy only trusts localhost + explicit CORS_ORIGIN(S).
    # Set true to restore the old permissive LAN behavior on a trusted network.
...
class RuntimeSettingsService:
    ...
    def render_environment(self) -> dict[str, str]:
        ...
        return {
            ...
            "CORS_ALLOW_ALL": _bool_env(system["cors_allow_all"]),
    ...
    def _process_env_secret(self, key: str) -> str:
        """Resolve a secret straight from the environment.

        Secrets are environment-only and must resolve on every deployment path,
        including Docker/Render where the entrypoint sets
        ``DEEPTUTOR_IGNORE_PROCESS_ENV_OVERRIDES=1`` to keep non-secret runtime
        config (ports, URLs, auth-enabled) JSON-driven. Unlike
        ``_process_env_value`` this deliberately ignores that flag and the
        internal-export dedup — a key only ever comes from the real env.
        """
        return _string(self.process_env.get(key, ""))
    ...
    def _apply_system_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("CORS_ALLOW_ALL"):
            payload["cors_allow_all"] = value
    ...
    def _apply_auth_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = self._normalize_auth(payload)
        # Env-only secret injected after normalize (never persisted). Reads the
        # env directly so it works on Render even with process overrides off.
        if value := self._process_env_secret("AUTH_PASSWORD_HASH"):
    ...
    def _apply_integrations_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = self._normalize_integrations(payload)
        # Env-only secret injected after normalize (never persisted). Reads the
        # env directly so it works on Render even with process overrides off.
        if value := self._process_env_secret("POCKETBASE_ADMIN_PASSWORD"):
    ...
    def _apply_mineru_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        payload = self._normalize_mineru_engine(payload)
        # Env-only secret injected after normalize (never persisted). Reads the
        # env directly so it works on Render even with process overrides off.
        if value := self._process_env_secret("MINERU_API_TOKEN"):
    ...
    def _apply_pageindex_process_overrides(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        if value := self._process_env_value("PAGEINDEX_API_BASE_URL"):
            payload["api_base_url"] = value
        payload = self._normalize_pageindex(payload)
        # Env-only secret injected after normalize (never persisted). Reads the
        # env directly so it works on Render even with process overrides off.
        if value := self._process_env_secret("PAGEINDEX_API_KEY"):
    ...
    def _normalize_pageindex(self, settings: dict[str, Any]) -> dict[str, Any]:
        return {
            "version": 1,
            # Env-only secret: never persisted to disk. Resolved from
            # PAGEINDEX_API_KEY in _apply_pageindex_process_overrides.
            "api_key": "",
    ...
    def _normalize_mineru_engine(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            or "https://mineru.net",
            # Env-only secret: never persisted to disk. Resolved from
            # MINERU_API_TOKEN in _apply_mineru_process_overrides.
            "api_token": "",
    ...
    def _normalize_system(self, settings: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "cors_allow_all": _coerce_bool(settings.get("cors_allow_all"), False),
    ...
    def _normalize_auth(self, settings: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            # Env-only secret: never persisted. From AUTH_PASSWORD_HASH.
            "password_hash": "",
    ...
    def _normalize_integrations(self, settings: dict[str, Any]) -> dict[str, Any]:
        return {
            ...
            # Env-only secret: never persisted. From POCKETBASE_ADMIN_PASSWORD.
            "pocketbase_admin_password": "",
        }

...
class SQLiteSessionStore:
    ...
    def __init__(self, db_path: Path | None = None, *, in_memory: bool = False) -> None:
        self.in_memory = in_memory
        self._keepalive: sqlite3.Connection | None = None
        if in_memory:
            # Ephemeral store for demo mode: a shared-cache in-memory DB that
            # never touches disk. ``_connect`` opens a fresh connection per op
            # and closes it, so with ``cache=shared`` the in-memory DB would be
    ...
    def _connect(self) -> Iterator[sqlite3.Connection]:
        ...
        conn = sqlite3.connect(self.db_path, uri=self.in_memory)

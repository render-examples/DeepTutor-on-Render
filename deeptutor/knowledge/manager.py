...
class KnowledgeBaseManager:
    ...
    def register_lightrag_server_kb(
        ...
        *,
        ...
        whose ``server_url`` the ``lightrag-server`` provider queries over HTTP.
        The API key is environment-only (``LIGHTRAG_API_KEY``) and is never
        stored on disk. The server owns indexing entirely. Callers should
        validate reachability with the probe helper first; this only guards
        basic invariants. Raises ``ValueError`` on a missing name/URL or a name
        clash.
        """
        name = (name or "").strip()

...
def build_completion_kwargs(
    *,
    ...
    Reasoning models (o1/o3/gpt-5) reject any non-default ``temperature`` with a
    400, so the requested value is passed through :func:`get_effective_temperature`
    which forces the model's fixed value when one applies. This mirrors the
    cloud_provider.py path; without it those models fail on the chat capability.
    """
    effective_temperature = get_effective_temperature(
        binding or "openai", model, requested_temp=temperature
    )

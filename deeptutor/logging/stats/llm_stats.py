...
# Shared pricing table (USD per 1K tokens) — single source of truth in
# deeptutor.config.constants so this logger and the research token tracker stay
# in sync. Re-exported here for backwards compatibility.
from deeptutor.config.constants import MODEL_PRICING
...
def get_pricing(model: str) -> dict[str, float]:
    ...
    for key, pricing in MODEL_PRICING.items():
        if key in model_lower or model_lower in key:
            return pricing
    return MODEL_PRICING.get("gpt-5.6-luna", {"input": 0.001, "output": 0.006})

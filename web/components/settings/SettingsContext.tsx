...
export type CatalogProfile = {
  ...
  provider?: string;
  base_url: string;
  // API keys are environment-only: never stored on disk or sent to/from the
  // browser. The server returns these read-only indicators instead.
  api_key_set?: boolean;
...
/**
 * Deep-clone the catalog and strip provider key fields from every profile so
 * they are never sent to the server. API keys are environment-only; the server
 * recomputes the read-only `api_key_set`/`api_key_env_var` indicators, so we
 * drop those too. Stripping `api_key` is the critical, defensive part.
 */
function catalogForSave(catalog: Catalog): Catalog {
  const clone = cloneCatalog(catalog);
  for (const service of Object.values(clone.services)) {
    for (const profile of service.profiles) {
      const p = profile as CatalogProfile & { api_key?: unknown };
      delete p.api_key;
      delete p.api_key_set;
      delete p.api_key_env_var;
    }
  }
  return clone;
}
  ...
  const saveCatalog = useCallback(async () => {
    ...
    try {
      const response = await apiFetch(apiUrl("/api/v1/settings/catalog"), {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ catalog: catalogForSave(draft) }),
      });

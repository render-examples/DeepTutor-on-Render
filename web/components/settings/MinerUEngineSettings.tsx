...
import EnvVarIndicator from "@/components/common/EnvVarIndicator";
import {
  SettingRow,
  SettingSection,
  inputClass,
...
type MinerUPayload = {
  ...
  /** Env var name holding the token (e.g. "MINERU_API_TOKEN"); read-only. */
...
export function MinerUEngineSettings() {
  ...
  const tokenEnvVar = payload?.api_token_env_var || "MINERU_API_TOKEN";
      ...
      {isCloud && (
        ...
        >
          ...
          <SettingRow
            title={t("API token")}
            description={t(
              "The API token is read from the environment and is never stored on disk.",
            )}
            control={
              <EnvVarIndicator
                envVar={tokenEnvVar}

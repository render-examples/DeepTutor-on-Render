...
export default function CreateKbModal({
  ...
  // A fresh URL invalidates a stale server connection test.
  useEffect(() => {
    setServerProbe(null);
  }, [serverUrl]);
  ...
  return (
    <div className="space-y-3">
      ...
      <div>
        ...
        <div className="flex flex-wrap items-center gap-x-2 gap-y-1 rounded-lg border border-[var(--border)] bg-[var(--muted)]/20 px-3 py-2 text-[12px] text-[var(--muted-foreground)]">
          <span>{t("Set via environment variable")}</span>
        ...
        <p className="mt-1 text-[11px] text-[var(--muted-foreground)]">
          {t(
            "If your server requires a key, set LIGHTRAG_API_KEY in the environment; it is resolved server-side and never sent from the browser.",
          )}
        </p>
      </div>

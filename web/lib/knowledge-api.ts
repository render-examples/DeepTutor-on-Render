...
export interface PageIndexConfig {
  api_base_url: string;
  api_key_set: boolean;
  /** Env var name holding the key (e.g. "PAGEINDEX_API_KEY"); read-only. */
...
export async function updatePageIndexConfig(payload: {
  // API keys are environment-only and no longer accepted from the browser.
  api_base_url?: string;
}): Promise<PageIndexConfig> {
...
export async function probeLightRagServer(payload: {
  ...
  // The API key is environment-only (LIGHTRAG_API_KEY); the server resolves it
  // from the environment and never accepts one from the browser.
  const res = await apiFetch(
...
export async function connectLightRagServer(payload: {
  ...
  // The API key is environment-only (LIGHTRAG_API_KEY); never sent from here.
  const res = await apiFetch(
